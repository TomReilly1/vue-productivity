...
const getFormData = () => {
    const proj: HTMLInputElement = <HTMLInputElement>document.getElementById("task-proj")
    const desc: HTMLTextAreaElement = <HTMLTextAreaElement>document.getElementById("task-desc")
    const status: HTMLSelectElement = <HTMLSelectElement>document.getElementById("task-sts")

    if (proj === null || desc === null || status == null) {
        return
    }

    const proj_val = proj.value
